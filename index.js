...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `SALVEEE @${num.split('@')[0]}\nBem vindo Ao puteiro*${mdata.subject}*\n*FIQUE A VONTS MEN:)*\n\n*LEIA AS REGRAS PRA NN SER REMOVIDO:D*`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FadhilGraphy = ["5511910592328@s.whatsapp.net"]
			...
			mess = {
				wait: '*[ WTF]* ⏳AGUARDE UM POUCO MANO...',
				success: '*[💯] Sucesso...*',
				error: {
			...
			const ownerNumber = [
			"5511910592328@s.whatsapp.net"//GANTI NOMOR LU
			]
			premium = [
			"5511910592328@s.whatsapp.net","6283102650464@s.whatsapp.net"//GANTI NOMOR YAG MAU DI PREM, GABISA NYIMPEN DI DATABASE!!
			]
...
/*********END PISAH MENU***********/
			  ...
			  case 'donate':
					...
					user.push(sender)
					uptime = process.uptime()
					teks = `⟩➢ *Nome do Bot* : ${me.name}\n⟩➢ *Numero do Bot* : @${me.jid.split('@')[0]}\n⟩➢ *prefix* : | ${prefix} |\n⟩➢ *Total de Block* : ${blocked.length}\n⟩➢ *Ativo desde* : ${kyun(uptime)}\n\n⟩➢ Total de pessoa: *${user.length}*Do utilizador\n⟩➢ *Youtube* : https://youtube.com/channel/UCalQOjX5BFqim6x8JyCMM-w`
				...
				case 'stikergift':
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`\`\`\`Falhou, no momento da conversão ${tipe} De stiker\`\`\``)
							})
							.on('end', function () {
								console.log('Finish')
								buff = fs.readFileSync(ran)
         ...
         case 'lirik':
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('5511910592328@s.whatsapp.net', options, text, {quoted: mek})
                    reply('As solicitações foram relatadas ao Cofs, as solicitações que podem sobrecarregar o proprietário não serão respondidas.')
                    break
				case 'meme':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
...
//creator
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
			if (args.length < 1) return reply(`${name} O que escrever, mana??`)
			reply(mess.wait)
					tulis = body.slice(7)
				  nama = tulis.split("/")[0];
					kelas = tulis.split("/")[1];
	   ...
	   case 'coli':
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 GABRIEL COFS 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`deleta todo o chat do ARR157-BOT\`\`\``)
					break
                                case 'bcgc':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *TRANSMISSÃO DO ARR157-BOT* ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão de sucesso')
					}
					break
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedidos aceitos, no edo tensei : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'promote':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido Recebido, @${mentioned[0].split('@')[0]} Você agora e um admin mas respeito membros comuns *${groupMetadata.subject}*`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				case 'delete':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedidos recebidos, : @${mentioned[0].split('@')[0]} Torne-se um membro comum ks`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'listadmins':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					break
				case 'toimg':
					if (!isQuotedSticker) return reply(' marque a figurinha lek pqp')
				...
				if (!isUser) return reply(mess.only.userB)
		         ...
		         case 'babi':
                    ...
                    const mmkk = `A MAIORIA DOS PORCOS AQUI ESTÃO @${gmik.replace(/@c.us/g, '')}`
          ...
          case 'snack':
			if (isBanned) return reply(mess.only.benned)    
             ...
             case 'ytmp4':
					...
					if (anu.error) return reply(anu.error)
					ytt = `「 *YOUTUBE MP4 DOWNLOADE* 」\n\n• Titulo : *${anu.title}*\n• *Tamanho:* ${anu.filesize}\n• *Descrição:* ${anu.desc}\n\n Espere um minuto 1 minuto talvez um pouco mais por causa do download do vídeo`
           ...
           case 'playmp3':
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `「 *PLAY MP3* 」\n*• Título:* ${anu.result.title}\n*• Fonte:* ${anu.result.source}\n*• Tamanho:* ${anu.result.size}\n\n*CALMA AE LEK TO ENVIANDO A MÚSICA PDP*`
...
// Akhir Fitur Premium 
	       ...
	       if (isBanned) return reply(mess.only.benned)
				if (!isUser) return reply(mess.only.userB)
					...
					if (anu.error) return reply(anu.error)
					teks = `*Título* : ${anu.title}\n\n Espere um minuto 1 minuto, talvez um pouco mais porque o download de vídeo`
			...
			case 'google':
               ...
               if (isBanned) return reply(mess.only.benned)    
				...
				default:
                  ...
                  reply(`Desculpe man *${pushname2}*, Comando *${prefix}${command}* nn registrado no menu! pfv chame o bot novamente *${prefix}menu*`)
                  }
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ARR157-BOT]','Yellow'), 'Comando não registrado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'yellow'))
